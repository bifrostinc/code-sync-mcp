...
@pytest.mark.asyncio
async def test_handle_verification_request_via_websocket():
    """Test handling a verification request from IDE (in standalone mode)."""
    connection_manager = make_connection_manager()
    push_id_from_message = str(uuid4())
    app_id = "test-app-id"
    deployment_id = "test-deployment-id"

    # Replace the verification_runner with a mock
    connection_manager.verification_runner.run_verification = AsyncMock()

    mock_ide_ws = AsyncMock(spec=WebSocket)
    mock_sidecar_ws = AsyncMock(spec=WebSocket)

    # Prepare TestConfig part of the verification request
    test_config_payload = ws_pb2.TestConfig(
        http_tests=[
            ws_pb2.HttpTest(
                method=ws_pb2.HttpTest.HttpMethod.GET,
                path="/health",
            )
        ],
        browser_tests=[
            ws_pb2.BrowserTest(workflow_steps=["click #login", "expect #dashboard"])
        ],
    )

    # Prepare the verification request message from IDE
    verification_request_payload = ws_pb2.VerificationRequest(
        push_id=push_id_from_message,
        tests=test_config_payload,
    )
    ide_message = ws_pb2.WebsocketMessage(
        message_type=ws_pb2.WebsocketMessage.MessageType.VERIFICATION_REQUEST,
        verification_request=verification_request_payload,
    )

    # Configure Sidecar mock to wait + disconnect
    async def sidecar_receive_gen() -> bytes:
        await asyncio.sleep(0.5)
        raise WebSocketDisconnect(code=1000, reason="Test finished")
    ...
    # Configure IDE mock to send one message then disconnect
    async def ide_ws_receive_gen():
        async def wait_and_disconnect():
            await asyncio.sleep(0.5)
            raise WebSocketDisconnect(code=1000, reason="Test finished")

        msg = ide_message.SerializeToString()
        mock_ide_ws.receive_bytes = wait_and_disconnect
        return msg
    ...
    try:
        await asyncio.wait_for(ide_task, timeout=1.0)
    except asyncio.TimeoutError:
        ide_task.cancel()
        await asyncio.gather(ide_task, return_exceptions=True)
        pytest.fail(
            "IDE task timed out during message processing for verification request"
        )
    except WebSocketDisconnect:
        pass  # Expected

    # Assert run_verification was called with correct arguments
    expected_tests_payload_dict = {
        "http_tests": [
            {
                "method": "GET",
                "path": "/health",
        ...
        "browser_tests": [
            {
                "workflow_steps": ["click #login", "expect #dashboard"],
            }
        ],
    }

    connection_manager.verification_runner.run_verification.assert_awaited_once()
    call_args = connection_manager.verification_runner.run_verification.call_args[1]
    assert call_args["user_id"] is None
    assert call_args["app_id"] == app_id
    assert call_args["push_id"] == push_id_from_message
    assert call_args["deployment_id"] == deployment_id
    assert call_args["tests_payload"] == expected_tests_payload_dict

    # Check that "in progress" response was sent
    mock_ide_ws.send_bytes.assert_called_once()
    sent_to_ide_bytes = mock_ide_ws.send_bytes.call_args[0][0]
    sent_to_ide_message = ws_pb2.WebsocketMessage()
    sent_to_ide_message.ParseFromString(sent_to_ide_bytes)
    ...
    assert (
        sent_to_ide_message.message_type
        == ws_pb2.WebsocketMessage.MessageType.VERIFICATION_RESPONSE
    )
    assert (
        sent_to_ide_message.verification_response.status
        == VerificationStatusPb.IN_PROGRESS
    )
