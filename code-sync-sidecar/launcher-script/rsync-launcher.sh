...
echo "Running rsync launcher script, watch_dir: ${WATCH_DIR} and app_root: ${APP_ROOT}"
COMMAND="sh -c \"$*\""
echo "Wrapping command: ${COMMAND}"
...
update_files() {
    ...
    if [ -z "$rsync_binary" ]; then
        echo "Warning: No executable rsync binary found in ${SIDECAR_DIR}"
        return 1
    fi

    echo "Syncing files from ${WATCH_DIR} to ${APP_ROOT} using ${rsync_binary}"
    ...
    if [ $rsync_exit_code -ne 0 ]; then
        echo "Error: rsync command failed with exit code ${rsync_exit_code}"
        return 1
    else
        echo "Successfully synced files to ${APP_ROOT}"
    fi
}
...
start_app() {
    ...
    if [ -f "$PUSH_ID_FILE" ]; then
        ...
        if [ -n "$PUSH_ID_VALUE" ]; then
            ...
            echo "Setting BIFROST_PUSH_ID='$BIFROST_PUSH_ID' from $PUSH_ID_FILE"
        else
            echo "Push ID file $PUSH_ID_FILE is empty, unsetting BIFROST_PUSH_ID"
            unset BIFROST_PUSH_ID
        fi
    else
        echo "Push ID file $PUSH_ID_FILE not found. Unsetting BIFROST_PUSH_ID."
        unset BIFROST_PUSH_ID
    fi
